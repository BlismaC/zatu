...
let playerName = "";
export let topKillerId = null; // Export topKillerId for client-side use
...
// --- Game Constants (Client-Side Visuals & Input) ---
// These constants are primarily for rendering and client-side input interpretation.
// The server has its own authoritative versions of game-logic related constants.
...
export const SWING_REACH = 40;     // Export (Client-side visual reach)
...
// These properties are for client-side rendering and do not dictate server-side game logic.
export const RESOURCE_PROPERTIES = {
    [RESOURCE_TYPES.WOOD]: {
        collectionAmount: 1, // Visual, server determines actual amount
        xpReward: 7.5,     // Visual, server determines actual XP
        collisionRadius: 100, // Visual, server determines actual collision
        hitRadius: 120,    // Visual, server determines actual hit detection
    },
...
// Wiggle Constants for resources (purely visual)
...
// Player Damage Visual Constants (purely visual)
...
// NEW: Chat Bubble Constants (purely visual)
...
// NEW: Dead Player Hide Delay (purely visual)
...
let mouseX = 0;
let mouseY = 0;
const keys = {}; // Stores current key states for sending to server
...
    if (me && me.isDead) {
        // Only emit respawn request if the player is dead
        socket.emit('respawn');
    }
});
...
            // Clear movement keys when chat is open to prevent accidental movement
...
// Function to send local chat messages to the server
...
            // Emit the message to the server; server will handle broadcasting to nearby players
...
        }
        // Clear all keys if player is dead to prevent ghost movement
        for (const key in keys) {
            keys[key] = false;
        }
        return;
...
    // Client-side initialization of player visual properties
...
        p.lastKnownHealth = MAX_HEALTH; // Track health for hit flash
        p.lastDamageTime = 0;
        p.damageWiggleX = 0;
        p.damageWiggleY = 0;
...
        p.deathTime = 0; // Initialize deathTime for client-side player object
    };

    // Initial game state received from server
    socket.on("init", (data) => {
        myId = socket.id;
...
    // Update player and resource states from server
    socket.on("player-moved", (data) => {
        const allPlayersData = data.players;
        const allResourcesData = data.resources;
        topKillerId = data.topKillerId; // NEW: Update topKillerId from server
...
                // Check if player just died (client-side visual effect trigger)
...
                // Apply damage wiggle and flash effect if health decreased
...
                // Preserve client-side visual states (e.g., smoothing, swing animation, chat message)
...
                // IMPORTANT: Ensure inventory is explicitly updated and includes gold
                // This ensures the client's inventory display is always in sync with the server.
                p.inventory = {
...
                    gold: serverPlayer.inventory.gold || 0,
                    kills: serverPlayer.inventory.kills || 0 // Ensure kills are updated
                };
                
                // Update player object with server data, preserving client-side visual states
                Object.assign(p, serverPlayer); 

                p.lastKnownHealth = serverPlayer.health; // Update last known health for next damage check
            } else {
                // New player joined
...
            }
        }
        // Remove players that are no longer present on the server
        for (const id in players) {
...
        // Update resources based on server data
...
            }
        }
        // Remove resources that are no longer present on the server
        for (const id in resources) {
            if (!allResourcesData[id]) {
                delete resources[id];
            }
        }
        // Update leaderboard whenever player data moves, passing the topKillerId
        updateLeaderboard(players, myId, topKillerId);
    });
...
    // Trigger client-side swing animation
...
            // Alternate hands for visual effect
...
        updateLeaderboard(players, myId, topKillerId); // Pass topKillerId
    });

    // Apply resource wiggle effect (visual only)
...
    // Listen for local chat messages from server to display bubbles
...
    // Send ping to server at regular intervals
    setInterval(() => {
...
// Applies a visual wiggle effect to resources
...
function update() {
    const me = players[myId];
    if (!me || me.isDead) return; // If player is dead, stop sending input
...
    // Always send client-input to the server. The server will validate and apply movement.
...
    // The server will validate cooldowns and hit detection.
    if (!isChatInputFocused && (isLeftMouseDown || isRightMouseDown) && !me.isDead) {
        socket.emit('player-swing');
    }

    // Client-side resource wiggle decay (purely visual)
    for (const id in resources) {
...
    lastTime = Date.now();
    
    // Update game state by sending input to server and processing server updates
    update();
    
    // Drawing (rendering) always happens, regardless of menu state,
    // so the canvas is always live in the background.
    // NEW: Pass topKillerId to the draw function
    draw(ctx, canvas, players, myId, resources, cameraX, cameraY, deltaTime, currentPing, CHAT_BUBBLE_DURATION, topKillerId);
...
        // Update HTML resource counter based on server-provided inventory
